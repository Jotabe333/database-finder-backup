...
type ExecutionState = "idle" | "running" | "success" | "error" | "confirm-cancel" | "confirm-execute";
...
echo.`;
  ...
  const canClose = execState !== "running" && execState !== "confirm-cancel" && execState !== "confirm-execute";
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-background/70 backdrop-blur-sm p-4">
      <div className="bg-card rounded-lg w-full max-w-[580px] max-h-[90vh] border border-border shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="p-4 space-y-3 overflow-y-auto">
          ...
          {/* Confirm execute for multiple banks */}
          {execState === "confirm-execute" && (
            <div className="rounded-md border border-primary/30 bg-primary/5 px-4 py-4 space-y-3">
              <div className="flex items-center gap-3">
                <AlertTriangle className="w-5 h-5 text-primary shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-foreground">Confirmar execução em lote</p>
                  <p className="text-[11px] text-muted-foreground">
                    Você selecionou <span className="font-semibold text-foreground">{entries.length} bancos</span> para backup. O processo será executado sequencialmente e pode demorar.
                  </p>
                </div>
              </div>
              <div className="rounded-md border border-border bg-background/50 max-h-[100px] overflow-y-auto">
                {entries.map((entry, i) => (
                  <div key={entry.id} className="flex items-center justify-between px-3 py-1.5 text-xs border-b border-border/30 last:border-0">
                    <span className="font-medium text-foreground">{i + 1}. {entry.name}</span>
                    <span className="font-mono text-[10px] text-muted-foreground">{entry.ip}</span>
                  </div>
                ))}
              </div>
              <div className="flex justify-end gap-1.5 pt-1">
                ...
                <button
                  className="flex items-center gap-1.5 px-4 py-1.5 rounded-md text-xs font-medium bg-primary text-primary-foreground hover:brightness-110 transition-all"
                  onClick={handleExecute}
                >
                  <Play className="w-3.5 h-3.5" />
                  Sim, executar {entries.length} bancos
                </button>
              </div>
            </div>
          )}
